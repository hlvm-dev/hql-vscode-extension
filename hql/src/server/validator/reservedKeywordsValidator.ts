    ...
    if (isList(expr)) {
      ...
      if (list.elements.length > 1 && 
          ...
          if (['fn', 'fx'].includes(keyword) && list.elements.length > 2 && isList(list.elements[2])) {
            ...
            this.checkFunctionParameters(document, paramList, diagnostics);
          }
      }
      ...
      if (list.elements.length >= 2 && isSymbol(list.elements[0])) {
        ...
        // Skip dot access/method calls
        if (fnName.includes('.')) {
          // Do nothing - dot access is valid
        }
        // Skip special forms and non-function calls
        else if (!['fn', 'fx', 'let', 'var', 'import', 'export', 'enum', 'class', 'struct'].includes(fnName)) {
